(function () {
  ...
  function vidaaNoty(text) {
    try {
      if (Lampa.Noty && typeof Lampa.Noty.show === 'function') {
        Lampa.Noty.show(String(text).slice(0, 160));
      }
    } catch (e) {}
  }

  function vidaaNativeAvailable() {
    try {
      return window.omi_platform && typeof window.omi_platform.sendPlatformMessage === 'function';
    } catch (e) {
      return false;
    }
  }

  function vidaaLaunchNative(url, title) {
    if (window.LampaVIDAANative && typeof window.LampaVIDAANative.native === 'function') {
      return window.LampaVIDAANative.native(url, title);
    }

    if (!vidaaNativeAvailable()) {
      vidaaNoty('[FX] VIDAA native API нет');
    ...
    vidaaNoty('[FX] native: ' + url.slice(0, 90));
    ...
    try {
      ...
      window.omi_platform.sendPlatformMessage(payload);
    } catch (e) {
      vidaaNoty('[FX] native error: ' + (e && e.message ? e.message : e));
      return Promise.resolve(false);
    }

    return new Promise(function (resolve) {
      setTimeout(function () {
        resolve(document.visibilityState === 'hidden');
      }, 2500);
    });
  }
      ...
      component.draw(items, {
        ...
        onEnter: function onEnter(item, html) {
          ...
          if (extra.file) {
            ...
            vidaaNoty('[FX] play: ' + String(extra.file).slice(0, 90));
    ...
    this.contextMenu = function (params) {
      params.html.on('hover:long', function () {
        function show(extra) {
          ...
          if (vidaaNativeAvailable() || (window.LampaVIDAANative && window.LampaVIDAANative.native)) {
            menu.push({
              title: Lampa.Lang.translate('player_lauch') + ' - VIDAA native',
              vidaa_native: true
            });
          }
          ...
          Lampa.Select.show({
            ...
            onSelect: function onSelect(a) {
              ...
              if (a.vidaa_native) {
                var nativeUrl = extra && extra.file;

                if (nativeUrl) {
                  vidaaLaunchNative(nativeUrl, params.element && params.element.title);
                } else {
                  vidaaNoty('[FX] нет URL для VIDAA native');
                }
